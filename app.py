...
try:
    ...
    st.error("Error: El archivo 'vehicles_us.csv' no se encuentra. Asegúrate de que el archivo existe en el mismo directorio que app.py y que ha sido subido a GitHub.")
    st.stop()

# --- PARA DEPURACIÓN: Muestra los nombres de las columnas para verificar ---
st.write("Columnas disponibles en el archivo CSV:", car_data.columns.tolist())
...
# Si la columna 'manufacturer' no existe, esta línea fallará
if 'manufacturer' in car_data.columns:
    ...
    st.error("Error: La columna 'manufacturer' no se encontró en el archivo CSV. Por favor, revisa el archivo 'vehicles_us.csv'.")
